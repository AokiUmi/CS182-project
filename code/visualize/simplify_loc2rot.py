...
class joints2smpl:
    ...
    def npy2smpl(self, npy_path):
        out_path = npy_path.replace('.npy', '_smpl.motion').replace('_','0').replace('joints','smpl')
        ...
        thetas, _ = self.joint2smpl(motions)  # [nframes, njoints, 3]
        print(thetas.shape)
    ...
    def joint2smpl(self, input_joints, init_params=None):
        ...
        thetas = new_opt_pose.reshape(self.batch_size, -1)# [bs, 72]
        ...
        thetas = torch.cat([root_loc, thetas], dim=-1)  # [1, 25, 6, 196]
...
if __name__ == '__main__':
    ...
    num_frames = np.load(params.input_path).shape[0]
    simplify = joints2smpl(num_frames, device_id=params.device, cuda=params.cuda)
