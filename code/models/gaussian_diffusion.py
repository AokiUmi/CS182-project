...
class MotionDiffusion(GaussianDiffusion):
    ...
    def __init__(self, use_timesteps, **kwargs):
        ...
        self.normalizer = MotionNormalizerTorch(dataset="interaction")
    ...
    def recover_from_ric(self, data, joints_num, global_rt=None):
        ...
        if global_rt is not None:
            global_rt = global_rt.to(positions.device)
            global_rot = global_rt[:,0].reshape(-1,1,1,1)
            global_t = global_rt[:,1:3].reshape(-1,1,1,1,2)
            # B,T,P,J,4
            global_r_rot_quat = torch.zeros(positions.shape[:-1] + (4,)).to(positions.device)
    ...
    def training_losses(
        ...
        relative = kwargs["model_kwargs"]["relative"]
        ...
        target= self.normalizer.forward(target, global_rt=True)
        # target[...,1,:] = self.normalizer.forward(target[...,1,:])

        kwargs["x_start"] = target.reshape(B,T,-1)
        items = super().training_losses(self._wrap_model(model), *args, **kwargs)
        prediction = items["pred"].reshape(B, T, -1, 263)
        ...
        weights["y_height"] = 0.01
        weights["joints"] = 0.1
        weights["poses"] = 1
        weights["vel"] = 1
        weights["contact"] = 0.01
        weights["g_y_rot"] = 0.01
        weights["g_relative_rot"] = 1
        weights["g_xz_trans"] = 0.01
        weights["g_joints"] = 1
        weights["contact_feet"] = 100.

        weights["relative_y_rot"] = 1
        weights["relative_xz_trans"] = 1
        weights["distance_constrain"] = 1
        ...
        losses["motion"] = self.masked_mse(prediction, target, mask)# * 0.01
        ...
        # target = self.normalizer.backward(target, global_rt=True)
        # prediction = self.normalizer.backward(prediction, global_rt=True)
        ...
        # #B,T,2,4
        # global_r_rot_quat = torch.zeros(pred_r_rot_quat[...,1:2,:].shape[:-1] + (4,)).to(pred_r_rot_quat.device)
        # global_r_rot_quat[..., 0] = torch.cos(relative[:,0]).reshape(-1, 1, 1)
        # global_r_rot_quat[..., 2] = torch.sin(relative[:,0]).reshape(-1, 1, 1)
        #
        #
        # pred_r_rot_quat[...,1:2,:] = qmul(qinv(global_r_rot_quat), qinv(pred_r_rot_quat[...,1:2,:]))
        # tgt_r_rot_quat[...,1:2,:] = qmul(qinv(global_r_rot_quat), qinv(tgt_r_rot_quat[...,1:2,:]))
        # pred_relative_rot = qmul(pred_r_rot_quat[...,1:2,:], pred_r_rot_quat[...,0:1,:])
        # tgt_relative_rot = qmul(tgt_r_rot_quat[...,1:2,:], tgt_r_rot_quat[...,0:1,:])
        # losses["g_relative_rot"] = self.masked_mse(pred_relative_rot[...,[0,2]], tgt_relative_rot[...,[0,2]], mask[...,0:1,:]) * weights["g_relative_rot"]
        # # losses["g_y_rot"] = self.masked_mse(pred_r_rot_angle, tgt_r_rot_angle, mask) * weights["g_y_rot"]
        ...
        #
        # if target.shape[-2]==2:
        #     pred_g_joints1 = self.recover_from_ric(prediction[...,0:1,:], joints_num=22).reshape(-1,22,3)
        #     pred_g_joints2 = self.recover_from_ric(prediction[...,1:2,:], joints_num=22, global_rt=relative).reshape(-1,22,3)
        #     tgt_g_joints1 = self.recover_from_ric(target[...,0:1,:], joints_num=22).reshape(-1,22,3)
        #     tgt_g_joints2 = self.recover_from_ric(target[...,1:2,:], joints_num=22, global_rt=relative).reshape(-1,22,3)
        #     # print(prediction[...,0:1,:].shape)
        #     # print(pred_g_joints1.shape)
        #
        #
        #     pred_distance_matrix = torch.cdist(pred_g_joints1.contiguous(), pred_g_joints2).reshape(mask.shape[:-2]+(1,-1,))
        #     tgt_distance_matrix = torch.cdist(tgt_g_joints1.contiguous(), tgt_g_joints2).reshape(mask.shape[:-2]+(1,-1,))
        #     # print(torch.cdist(pred_g_joints1.contiguous(), pred_g_joints2)[0] - torch.cdist(tgt_g_joints1.contiguous(), tgt_g_joints2)[0])
        #     losses["distance_constrain"] = self.masked_mse(pred_distance_matrix, tgt_distance_matrix, mask[...,0:1,:]) * weights["distance_constrain"]
