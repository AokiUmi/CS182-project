...
# def recover_root_rot_pos(data, relative=None):
#     rot_vel = data[..., 0]
...
#     r_rot_quat[..., 0] = torch.from_numpy(np.cos(rot_vel[...,2]).numpy())
#     r_rot_quat[..., 2] = torch.from_numpy(np.sin(rot_vel[...,2]).numpy())
#
#
...
#     print(r_pos[0])
#     # r_pos[0] = qrot(r_rot_quat[0], r_pos[0])
#
...
def recover_from_ric(data, joints_num, relative=None, global_rt=False):


    if global_rt:
        r_rot_quat = torch.zeros(data.shape[:-1] + (4,)).to(data.device)
        r_rot_quat[..., 0] = torch.cos(data[..., 0])
        r_rot_quat[..., 2] = torch.sin(data[..., 0])
        r_pos = torch.cat([data[..., 1:2], data[..., 3:4], data[..., 2:3]], dim=-1)
    else:
        r_rot_quat, r_pos = recover_root_rot_pos(data)
    ...
    if relative is not None and not global_rt:
        global_rot = relative[0][None]
        global_t = relative[1:3][None]
